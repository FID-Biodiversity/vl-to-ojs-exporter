...
from ojs.xmlgenerator import OjsArticle, OjsIssue, OjsXmlGenerator
...
class TestXmlGeneration:
    def test_issue_xml_generation(self, visual_library):
        xml_test_file = "{base_dir}/generator-test-issue.xml".format(
    ...
    def test_article_xml_generation(self):
        xml_test_file = "{base_dir}/generator-test-article.xml".format(
            base_dir=TEST_DATA_DIRECTORY
        )
        ...
        assert (
            ojs_article.title
            == "Flinzschiefer des Bergischen Landes und ihre Beziehungen zum Massenkalk"
        )
    ...
    def test_multi_author_article(self):
        article_id = "10902111"
        expected_outcome_file = "{base_dir}/multi-author-article-outcome.xml".format(
            base_dir=TEST_DATA_DIRECTORY
        )
    ...
    def test_pre_ojs_3_2_schema_article(self):
        ...
        expected_outcome_file = (
            "{base_dir}/pre_ojs_3_2_schema-article-outcome.xml".format(
                base_dir=TEST_DATA_DIRECTORY
            )
        )
        configurator = MockConfigurator()
        ...
        validate_ojs_native_xsd_consistency(
            generated_article_xml_string, pre_ojs32_schema=True
        )

    def test_pre_ojs_3_2_schema_issue(self):
        issue_id = "10801067"
        expected_outcome_file = (
            "{base_dir}/pre_ojs_3_2_schema-issue-outcome.xml".format(
                base_dir=TEST_DATA_DIRECTORY
            )
        )
        configurator = MockConfigurator()
    ...
    def test_rooting_of_every_issue_in_issues_tags(self):
        volume_id = "10801960"
        expected_outcome_file = (
            "{base_dir}/pre_ojs_3_2_schema-issue-in-issues-outcome.xml".format(
                base_dir=TEST_DATA_DIRECTORY
            )
        )
        configurator = MockConfigurator()
    ...
    def test_publication_year(self):
        ...
        assert (
            vl_article.subtitle
            == "* 31.Oktober 1881 in Trier, † 11.März 1968 in Köln . mit 1 Tafel"
        )
    ...
    def test_author_with_title(self):
        ...
        vl_article, xml_generator = create_vl_object_and_xml_generator(
            article_id, pre_3_2_schema=True
        )
        ojs_article = xml_generator.convert_vl_objecto_to_ojs_object(vl_article)
    ...
    def test_article_in_non_configured_language(self):
        ...
        assert (
            english_publication.find("title").text
            == "composition of species of the Asian Clams Corbicula "
        ...
        assert (
            german_publication.find("title").text
            == "Artzusammensetzung von Körbchenmuscheln Corbicula "
            "im Niederrhein"
        )
    ...
    def test_set_issue_title(self):
        ...
        assert (
            xml_soup.issue_identification.title.text
            == "Siebengebirge und Rodderberg : Beiträge zur Biologie "
    ...
    def test_multilanguage_titles(self):
        issue_id = "10804777"
        vl_issue, xml_generator = create_vl_object_and_xml_generator(
            issue_id, pre_3_2_schema=True
        )
        xml_generator.xml_configuration.change_configuration_value(
            "add_title_to_issue", True
        )
        ...
        assert (
            english_issue_title.text
            == "Geology and paleontology of the Devonian and Tertiary at the ICE line in the "
        ...
        assert (
            multi_language_article.find("title", {"locale": "de_DE"}).text
        ...
        assert (
            multi_language_article.find("title", {"locale": "en_US"}).text
...
def validate_ojs_native_xsd_consistency(xml_string, pre_ojs32_schema=False):
    ...
    if pre_ojs32_schema:
        xsd_files_path = "{xsd_files_path}/3.1.2-1".format(
            xsd_files_path=xsd_files_path
        )

    ojs_native_xsd_file_path = "{xsd_path}/ojs-native.xsd".format(
        xsd_path=xsd_files_path
    )
